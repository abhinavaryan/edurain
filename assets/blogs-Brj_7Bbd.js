import{ht as e,mt as t,pt as n,vt as r}from"./vendor-BpCc1Cic.js";import{r as i}from"./main-Ck-knwZ9.js";import{t as a}from"./courses-BKPhEzz0.js";var o=[`All Blogs`,`Study Hacks`,`JEE Mains PYQ`,`NEET Test Series`,`CBSE Class 10 PYQ`,`CBSE Class 12 PYQ`,`Foundation 9th-10th`],s=[];function c(){return`
    ...
    `}async function u(){let o=document.getElementById(`er-blog-search-input`),c=document.getElementById(`er-tags-container`),u=document.getElementById(`er-blogs-grid-container`),d=document.getElementById(`blog-list-view`),f=document.getElementById(`blog-reader-view`),p=document.getElementById(`pw-reader-main-content`),m=document.getElementById(`pw-reader-related-container`);if(!u||!f)return;let h=`All Blogs`,g=``;try{s=(await n(e(r(i,`blogs`),t(`date`,`desc`)))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>e.status===`published`);let a=[...new Set(s.map(e=>e.category).filter(e=>e))];c&&(c.innerHTML=`
            ...
            `).join(``)}let g=document.getElementById(`pw-sidebar-banner-container`);if(g)if(n.sidebarBannerImage){let e=n.sidebarBannerLink||`#`,t=e.includes(`youtube.com`)||e.includes(`youtu.be`)?`
                ...
                `,g.style.display=`block`}else g.style.display=`none`,g.innerHTML=``;p.innerHTML=`
        ...
        `;let _=s.filter(e=>e.id!==n.id).slice(0,3);m.innerHTML=`
        ...
        `,d.style.display=`none`,f.style.display=`block`;let b=document.getElementById(`blogs`);b?b.scrollIntoView({behavior:`smooth`}):window.scrollTo({top:0,behavior:`smooth`}),document.getElementById(`btn-bc-blogs`)?.addEventListener(`click`,y),document.getElementById(`btn-close-modal-bottom`)?.addEventListener(`click`,y),m.querySelectorAll(`.btn-open-blog`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.getAttribute(`data-blog-id`);n&&v(n)})})}function y(){f.style.display=`none`,d.style.display=`block`,document.title=`IIT JEE, NEET & Foundation Blogs | EduRain`;let e=document.querySelector(`meta[name="description"]`);e&&(e.content=`Read blogs on IIT JEE preparation, NEET exam strategy, and Foundation (6th-10th) study guides`);let t=document.getElementById(`blogs`);t&&t.scrollIntoView({behavior:`smooth`})}function b(){u.querySelectorAll(`.er-pw-blog-card`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation();let n=e.getAttribute(`data-blog-id`);n&&v(n)})})}}export{u as initBlogs,c as renderBlogs};