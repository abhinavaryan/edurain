import{t as e}from"./main-DpYkbAp0.js";function t(){return`
    ...
    `}function n(){let t=document.getElementById(`contact-form`),n=document.getElementById(`form-message`);t&&t.addEventListener(`submit`,async r=>{r.preventDefault();let i=document.getElementById(`contact-name`).value.trim(),a=document.getElementById(`contact-email`).value.trim(),o=document.getElementById(`contact-message`).value.trim();if(i.length<2){n.textContent=`Please enter your name (min 2 characters).`,n.className=`form-message error`;return}if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a)){n.textContent=`Please enter a valid email address.`,n.className=`form-message error`;return}if(o.length<10){n.textContent=`Message must be at least 10 characters.`,n.className=`form-message error`;return}let s=document.getElementById(`contact-submit`);s.disabled=!0,s.textContent=`Sending...`;try{await e(i,a,o);try{let e=new URLSearchParams;e.append(`name`,i),e.append(`email`,a),e.append(`message`,o),await fetch(`https://script.google.com/macros/s/AKfycbxspWFgxEFlxdDa1O5GXLHwjeBAsytPeAwAWpOqTlum_hQ0_P4_wIHtcj-HPMTZrJNY/exec`,{method:`POST`,body:e,mode:`no-cors`})}catch(e){console.error(`Google Sheets saving error:`,e)}n.textContent=`✅ Thanks for your enquiry! Our consultant will connect with you shortly.`,n.className=`form-message success`,t.reset()}catch{n.textContent=`❌ Failed to send message. Please try again.`,n.className=`form-message error`}finally{s.disabled=!1,s.textContent=`Send Message`}})}export{n as initContact,t as renderContact};