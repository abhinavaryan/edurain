function e(){return`
    ...
    `}function t(){let e=document.querySelectorAll(`.er-count`),t=!1;function n(e){let t=parseInt(e.dataset.target,10),n=e.dataset.suffix||``,r=0;function i(e){return t>=1e6?(e/1e6).toFixed(0)+n:e.toLocaleString(`en-IN`)+n}e.textContent=i(0);let a=setInterval(()=>{r=Math.min(r+Math.ceil(t/80),t),e.textContent=i(r),r>=t&&(clearInterval(a),e.textContent=i(t),e.style.animation=`erCountPing 0.4s ease-out`,setTimeout(()=>{e.style.animation=``},400))},25)}let r=document.getElementById(`er-impact`);r&&(new IntersectionObserver(r=>{r.forEach(r=>{r.isIntersecting&&!t&&(t=!0,e.forEach(e=>n(e)))})},{threshold:.25}).observe(r),document.querySelectorAll(`.er-impact-card`).forEach(e=>{let t=e.querySelector(`.er-impact-bar`);e.addEventListener(`mouseenter`,()=>{t&&t.classList.add(`er-bar-expanded`),e.style.transform=`translateY(-6px)`}),e.addEventListener(`mouseleave`,()=>{t&&t.classList.remove(`er-bar-expanded`),e.style.transform=`translateY(0)`})}))}export{t as initImpact,e as renderImpact};