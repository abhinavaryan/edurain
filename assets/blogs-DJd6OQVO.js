import{ht as e,mt as t,pt as n,vt as r}from"./vendor-BpCc1Cic.js";import{r as i}from"./main-BoNJnEQd.js";import{t as a}from"./courses-BKPhEzz0.js";var o=[`All Blogs`,`Study Hacks`,`JEE Mains PYQ`,`NEET Test Series`,`CBSE Class 10 PYQ`,`CBSE Class 12 PYQ`,`Foundation 9th-10th`],s=[];function c(){return`
    ...
    `}async function u(){let o=document.getElementById(`er-blog-search-input`),c=document.getElementById(`er-tags-container`),u=document.getElementById(`er-blogs-grid-container`),d=document.getElementById(`blog-list-view`),f=document.getElementById(`blog-reader-view`),p=document.getElementById(`pw-reader-main-content`),m=document.getElementById(`pw-reader-related-container`);if(!u||!f)return;let h=`All Blogs`,g=``;try{s=(await n(e(r(i,`blogs`),t(`date`,`desc`)))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>e.status===`published`);let a=[...new Set(s.map(e=>e.category).filter(e=>e))];c&&(c.innerHTML=`
            ...
            `),_();let o=window.location.pathname.split(`/`);if(o.length>2&&o[1]===`blogs`){let e=o[2];v(e,!1)}else{let e=window.location.hash.split(`?`)[0].split(`/`);if(e.length>1&&e[0]===`#blogs`){let t=e[1];v(t,!1)}}}catch(e){console.error(`Error fetching blogs:`,e),u.innerHTML=`<div style="text-align: center; padding: 2rem; color: #ef4444;">Failed to load blogs. Please try again later.</div>`}function _(){let e=s.filter(e=>{let t=Array.isArray(e.tags)?e.tags.map(e=>e.toLowerCase()):(e.tag||``).toLowerCase().split(`,`).map(e=>e.trim()),n=h===`All Blogs`||e.category&&e.category.toLowerCase().includes(h.toLowerCase())||t.includes(h.toLowerCase())||e.title&&e.title.toLowerCase().includes(h.toLowerCase()),r=!g||e.title&&e.title.toLowerCase().includes(g.toLowerCase())||e.excerpt&&e.excerpt.toLowerCase().includes(g.toLowerCase())||e.category&&e.category.toLowerCase().includes(g.toLowerCase());return n&&r});u.innerHTML=l(e),b()}o&&o.addEventListener(`input`,e=>{g=e.target.value.trim(),_()}),c&&c.addEventListener(`click`,e=>{let t=e.target.closest(`.er-tag-pill`);t&&(c.querySelectorAll(`.er-tag-pill`).forEach(e=>e.classList.remove(`active`)),t.classList.add(`active`),h=t.getAttribute(`data-tag`),_())});function v(e,t=!0){let n=s.find(t=>t.id===e||t.slug===e);if(!n)return;if(t){let e=n.slug||n.id;window.history.pushState({},``,`/blogs/${e}`)}let r=`Recently`;n.date&&(r=(n.date.toDate?n.date.toDate():new Date(n.date)).toLocaleDateString(`en-IN`,{year:`numeric`,month:`long`,day:`numeric`}));let i=``;i=Array.isArray(n.content)?n.content.map(e=>e.type===`heading`?`<h3 class="pw-article-h3">${e.text}</h3>`:`<p class="pw-article-p">${e.text}</p>`).join(``):`<div class="quill-content">${n.content||`<p class="pw-article-p">${n.excerpt}</p>`}</div>`;let o=document.createElement(`div`);o.innerHTML=i;let c=o.querySelectorAll(`a[href*="youtube.com"], a[href*="youtu.be"]`),l=!1;c.forEach(e=>{let t=e.querySelector(`img`);if(t&&!t.parentElement.classList.contains(`yt-play-overlay-wrapper`)){let e=document.createElement(`div`);e.className=`yt-play-overlay-wrapper`,e.style.position=`relative`,e.style.display=`inline-block`,e.style.width=`100%`,e.style.textAlign=`center`,t.parentNode.insertBefore(e,t),e.appendChild(t);let n=document.createElement(`div`);if(n.className=`yt-play-overlay`,n.style.cssText=`position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 80px; height: 80px; background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 32px rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.3); transition: all 0.3s ease; pointer-events: none;`,n.innerHTML=`<svg viewBox="0 0 24 24" width="40" height="40" fill="#ef4444" style="margin-left: 4px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2));"><path d="M8 5v14l11-7z"/></svg>`,e.appendChild(n),!l){let e=document.createElement(`style`);e.textContent=`.yt-play-overlay-wrapper:hover .yt-play-overlay { transform: translate(-50%, -50%) scale(1.1) !important; background: rgba(255,255,255,0.25) !important; border-color: rgba(255,255,255,0.5) !important; }`,o.prepend(e),l=!0}}}),i=o.innerHTML;let u=n.seo&&n.seo.metaTitle&&n.seo.metaTitle.trim()?n.seo.metaTitle.trim():n.title||`EduRain Blog`;document.title=u;let h=n.seo&&n.seo.metaDescription&&n.seo.metaDescription.trim()?n.seo.metaDescription.trim():n.excerpt||`Read this insightful blog on EduRain`,g=document.querySelector(`meta[name="description"]`);g&&(g.content=h);let _=window.location.origin+`/blogs/${n.slug||n.id}`,b=n.coverImage||window.location.origin+`/images/og-banner.png`,x=document.querySelector(`meta[property="og:title"]`);x&&(x.content=u);let S=document.querySelector(`meta[property="og:description"]`);S&&(S.content=h);let C=document.querySelector(`meta[property="og:url"]`);C&&(C.content=_);let w=document.querySelector(`meta[property="og:image"]`);w&&(w.content=b);let T=document.querySelector(`meta[name="twitter:title"]`);T&&(T.content=u);let E=document.querySelector(`meta[name="twitter:description"]`);E&&(E.content=h);let D=document.getElementById(`pw-sidebar-courses-list`);if(D){let e=[],t=[];Array.isArray(n.recommendedCourses)?t=n.recommendedCourses.map(e=>String(e).trim()):typeof n.recommendedCourses==`string`&&n.recommendedCourses.trim()!==``&&(t=n.recommendedCourses.split(`,`).map(e=>e.trim())),t.length>0&&(e=a.filter(e=>t.includes(String(e.id)))),e.length===0&&(e=a.slice(0,3)),D.innerHTML=e.map(e=>`
                <div class="pw-sidebar-course-card">
                    <img loading="lazy" src="${e.thumbnail}" alt="${e.title}" class="pw-sidebar-course-img" />
                    <div class="pw-sidebar-course-info">
                        <h4 class="pw-sidebar-course-title">${e.title}</h4>
            ...
            `).join(``)}let O=document.getElementById(`pw-sidebar-banner-container`);if(O)if(n.sidebarBannerImage){let e=n.sidebarBannerLink||`#`,t=e.includes(`youtube.com`)||e.includes(`youtu.be`)?`
                ...
                `,O.style.display=`block`}else O.style.display=`none`,O.innerHTML=``;p.innerHTML=`
        ...
        `;let k=s.filter(e=>e.id!==n.id).slice(0,3);m.innerHTML=`
        ...
        `,d.style.display=`none`,f.style.display=`block`;let A=document.getElementById(`blogs`);A?A.scrollIntoView({behavior:`smooth`}):window.scrollTo({top:0,behavior:`smooth`}),document.getElementById(`btn-bc-blogs`)?.addEventListener(`click`,y),document.getElementById(`btn-close-modal-bottom`)?.addEventListener(`click`,y),m.querySelectorAll(`.btn-open-blog`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.getAttribute(`data-blog-id`);n&&v(n)})})}function y(){f.style.display=`none`,d.style.display=`block`,document.title=`IIT JEE, NEET & Foundation Blogs | EduRain`;let e=document.querySelector(`meta[name="description"]`);e&&(e.content=`Read blogs on IIT JEE preparation, NEET exam strategy, and Foundation (6th-10th) study guides`);let t=document.getElementById(`blogs`);t&&t.scrollIntoView({behavior:`smooth`})}function b(){u.querySelectorAll(`.er-pw-blog-card`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),t.stopPropagation();let n=e.getAttribute(`data-blog-id`);n&&v(n)})})}}export{u as initBlogs,c as renderBlogs};